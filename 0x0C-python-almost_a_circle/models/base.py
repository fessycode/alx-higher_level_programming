        ...
        try:
            with open(filename, "r", newline="", encoding="utf-8") as fi:
                if cls.__name__ == "Rectangle":
                    ...
                    fieldnames = ["id", "size", "x", "y"]
                list_dictss = csv.DictReader(fi, fieldnames=fieldnames)
                list_dictss = [dict([k, int(v)] for k, v in d.items())
                              for d in list_dictss]
                return [cls.create(**d) for d in list_dictss]
        except IOError:
            return []
